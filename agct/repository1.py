...
class TableDef:
    ...
    columns: field(init=False)
    full_file_name: field(init=False)
...
class RepoSessionContext:
    ...
    def __init__(self, data_folder_root: str = "."):
        self._data_folder = os.path.join(data_folder_root, DATA_FOLDER)
...
class VariantEffectSourceCache(ParameterizedSingleton):
    """
    Caches the variant csv file in a dataframe. Implements the singleton
    pattern to ensure there is only one instance of the cached dataframe.
    We use an _init_once method rather than the normal __init__ method
        ...
        if self._data_frame is None:
            with self._lock:
                if self._data_frame is None:
                    self._data_frame = pd.read_csv(
                        os.path.join(
                            self._data_folder_root,
                            VARIANT_EFFECT_SOURCE_TABLE_DEF.full_file_name))
        return self._data_frame
...
class VariantRepository:
    ...
    def get(self, gene_symbols: List[str] = None,
        ...
        where_clause = build_dataframe_where_clause(
            {"ALLELE_FREQUENCY": [allele_frequency_operator, allele_frequency]})
        ...
        if gene_symbols is not None and len(gene_symbols) > 0:
            variant_df = filter_dataframe_by_list(variant_df, gene_symbols,
                                                  ['GENE_SYMBOL'],
                                                  include_genes)
        if variant_ids is not None and len(variant_ids) > 0:
            variant_df = filter_dataframe_by_list(variant_df, variant_ids,
                                                  VARIANT_PK_COLUMNS,
                                                  include_variant_ids)
        return variant_df
...
class VariantEffectLabelRepository:
    ...
    def get(self, task_name: str, gene_symbols: List[str] = None,
            include_genes: bool = None,
            variant_ids: pd.DataFrame = None,
            include_variant_ids: bool = None,
            allele_frequency_operator: str = None,
        ...
        label_df = self._cache.get_data_frame(task_name)
        if variant_ids is not None and len(variant_ids) > 0:
            label_df = filter_dataframe_by_list(label_df, variant_ids,
                                                VARIANT_PK_COLUMNS,
                                                include_variant_ids)
        variant_df = self._variant_repo.get(
            gene_symbols, include_genes,
            allele_frequency_operator=allele_frequency_operator,
            allele_frequency=allele_frequency)
        merge_df = label_df.merge(variant_df, how=""
...
class VariantEffectScoreRepository:
    ...
    def get(self, task_name: str,
        ...
        if variant_ids is not None and len(variant_ids) > 0:
            score_df = filter_dataframe_by_list(score_df, variant_ids,
                                                VARIANT_PK_COLUMNS,
                                                include_variant_ids)
        variant_df = self._variant_repo.get(
            gene_symbols, include_genes,
...
class VariantDAL:
    """
    Data access library class for accessing the raw repository data which
    can be stored in files or in a database. These details are
    hidden from caller.
    """

    def __init__(self, data_folder: str):
        self._data_folder = data_folder
        self._variant_repo = VariantRepository(data_folder)
        self._variant_task_repo = VariantTaskRepository(data_folder)

    def get_tasks(self) -> pd.DataFrame:
        return self._variant_task_repo.data_frame.copy(deep=True)

    def get_variant_effect_sources(self) -> pd.DataFrame:
        df = pd.read_csv(os.path.join(self._data_folder,
                                      "variant_effect_source.csv"))
        return df

    def get_variants(self, task_name: str, gene_symbols: List[str] = None,
                     include_genes: bool = None,
                     variant_ids: List[VariantId] = None,
                     include_variant_ids: bool = None,
                     allele_frequency_operator: str = None,
                     allele_frequency: float = None,
        ...
        filter_name : str, optional
            ...
            returned.
        """
        # variant_df = VariantCache().data_frame
